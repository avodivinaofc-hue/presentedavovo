-- Fix all remaining functions that still need search path
...
CREATE OR REPLACE FUNCTION public.get_all_users_admin()
 RETURNS TABLE(id uuid, email text, full_name text, created_at timestamp with time zone, updated_at timestamp with time zone, is_premium boolean, is_admin boolean, total_readings integer, disrupty_customer_id text, last_login timestamp with time zone, email_verified boolean, birth_date date, source text, cosmic_energy integer, max_cosmic_energy integer, last_energy_regen timestamp with time zone, energy_regen_rate integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  ...
  -- Log administrative action
  PERFORM admin_action_log('list_users', NULL, jsonb_build_object('users_count', (SELECT COUNT(*) FROM user_accounts)));
...
CREATE OR REPLACE FUNCTION public.admin_action_log(action_type text, target_id uuid, details jsonb DEFAULT '{}'::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  INSERT INTO user_actions (
    ...
    details,
    auth.uid(),
    (SELECT full_name FROM user_accounts WHERE auth_user_id = auth.uid()),
    NOW()
  );
END;
$function$;
